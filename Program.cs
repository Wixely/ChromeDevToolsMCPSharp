...
using ChromeDevToolsMCPSharp.Hosting;
...
namespace ChromeDevToolsMCPSharp;
...
public static class Program
{
    public static int Main(string[] args)
    {
        ...
        Log.Logger = new LoggerConfiguration()
            ...
            .WriteTo.Console()
            .WriteTo.File(
                Path.Combine(contentRoot, "logs", "chromedevmcp-bootstrap-.log"),
        ...
        try
        {
            ...
            builder.Configuration
                ...
                .AddJsonFile(ResolveConfigFile(contentRoot, "ChromeDevToolsMCPSharp.json"), optional: true, reloadOnChange: true)
                .AddJsonFile(ResolveConfigFile(contentRoot, $"ChromeDevToolsMCPSharp.{builder.Environment.EnvironmentName}.json"), optional: true, reloadOnChange: true)
                .AddJsonFile(ResolveConfigFile(contentRoot, "ChromeDevToolsMCPSharp.Local.json"), optional: true, reloadOnChange: true)
                .AddEnvironmentVariables()
                .AddEnvironmentVariables(prefix: "CHROMEDEVMCP_")
                .AddCommandLine(args);
            ...
            builder.Services.Configure<ChromeDevToolsOptions>(
                builder.Configuration.GetSection(ChromeDevToolsOptions.SectionName));
            builder.Services.Configure<ServerOptions>(
                builder.Configuration.GetSection(ServerOptions.SectionName));

            builder.Services.AddSingleton<ChromeDevToolsService>();
            ...
            var chrome = app.Services.GetRequiredService<ChromeDevToolsService>();
            var attachTarget = chrome.Options.WebSocketEndpoint
                ?? chrome.Options.BrowserUrl
                ?? (chrome.Options.AutoLaunch
                    ? $"auto-launch (headless={chrome.Options.Headless})"
                    : "(unset)");
            LogStartup(
                ...
                contentRoot,
                $"Read-only: {chrome.IsReadOnly}",
                $"Script eval disabled: {chrome.Options.DisableScriptEvaluation}",
                $"Chrome attach: {attachTarget}");
            ...
            {
                status = "ok",
                server = "ChromeDevToolsMCPSharp",
                path = server.Path,
                readOnly = chrome.IsReadOnly,
                disableScriptEvaluation = chrome.Options.DisableScriptEvaluation,
